...
viagens <- read.csv(
  ...
  file = "E:/R/Datasets/2019_Viagem.csv",
  sep = ';',
  dec = ','
)
...
# str vreifica a quantidade de ocorrencias da coluna situação
...
#Criando o gráfico
ggplot(p2, aes(x = reorder(destino, valor), y = valor))+ #graficos de barras
  geom_bar(stat = "identity", fill = "#0ba791")+ #cor
  geom_text(aes(label = valor), vjust = 0.3, size = 3)+ #tesxto no grafico
  coord_flip()+
...
#acima contamos o distinto do data frame
